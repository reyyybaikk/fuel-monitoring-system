...
export default function ReportsPage() {
  ...
  // 1. Ambil daftar armada untuk dropdown
  ...
  // 2. Ambil DATA ASLI transaksi untuk isi preview laporan (Live Sync)
    ...
    queryFn: async () => {
      const response = await api.get('/api/fuel-transactions/history', {
        params: {
          ...
          end_date: endDate,
          limit: 50 // Batasi untuk preview agar ringan
        }
      });
  ...
  const handleExportPDF = async () => {
    ...
    const loadingToast = toast.loading('Menyusun Berita Acara & Mengunduh Bukti Visual...');
    try {
      ...
      link.href = url;
      link.setAttribute('download', `Laporan_BBM_${startDate}.pdf`);
    ...
    <div className="space-y-4 font-sans select-none pb-12">
      ...
      <div className="bg-white p-4 rounded-[8px] border border-border shadow-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5 font-medium italic">Sinkronisasi otomatis dengan Database Supabase.</p>
        </div>
      </div>
        ...
        <Card className="xl:col-span-4 rounded-[8px] border border-border bg-white shadow-sm p-4 space-y-4">
          <div className="space-y-4">
            <div className="p-3 bg-pln-iceBlue/40 border border-pln-cyan/10 rounded-[6px]">
              ...
              <span className="text-xs font-black text-pln-darkBlue uppercase">{userProfile?.region}</span>
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-bold text-muted-foreground uppercase">Pilih Armada</label>
              <div className="flex flex-col gap-2 p-3 bg-slate-50 border border-border rounded-[8px]">
                <input
                  type="text"
                  placeholder="Cari pelat..."
                  value={vehicleSearch}
                  onChange={(e) => setVehicleSearch(e.target.value)}
                  className="w-full h-9 px-3 text-xs bg-white border border-border rounded-[4px] font-mono outline-none focus:border-pln-cyan"
                />
                <select
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">Mulai</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">Selesai</label>
            ...
            <Button
              ...
              className="w-full h-11 text-xs font-black bg-pln-darkBlue text-white hover:bg-pln-darkBlue/90 rounded-[6px] shadow-lg flex items-center justify-center gap-2.5 active:scale-95 transition-all mt-4"
            >
          ...
          <div className="w-full max-w-[210mm] mx-auto bg-white shadow-2xl p-[15mm] font-sans text-[#0b1c30] min-h-[297mm] flex flex-col relative overflow-hidden">
            ...
            <div className="space-y-1 mb-8 border-b border-slate-300 pb-4">
               <div className="flex text-xs">
                  ...
                  <span className="font-black uppercase">: {userProfile?.region ? `UL ${userProfile.region.replace('Unit Layanan ', '')}` : 'UPKAL2 REGIONAL'}</span>
               </div>
               <div className="flex text-xs">
                 ...
                 <table className="w-full text-left border-collapse border border-slate-400 text-[8px]">
                    <thead>
                      <tr className="bg-slate-50 font-bold text-slate-800 uppercase border-b-2 border-slate-400">
                        ...
                        <th className="p-1.5 border-r border-slate-400">Odometer</th>
                        <th className="p-1.5 border-r border-slate-400">Bar</th>
                    ...
                    <tbody className="divide-y divide-slate-300">
                      {reportTransactions.map((tx: any, index: number) => (
                        <tr key={tx.id}>
                          ...
                          <td className="p-1.5 border-r border-slate-300 font-black">{tx.license_plate}</td>
                          <td className="p-1.5 border-r border-slate-300 font-mono">{tx.odometer} Km</td>
            ...
            {/* 3. LAMPIRAN FOTO ASLI (PER TRANSAKSI) */}
            <div className="space-y-8">
               <h3 className="text-[10px] font-black text-slate-800 uppercase tracking-widest border-b-2 border-slate-800 pb-1 inline-block">
                 LAMPIRAN BUKTI FISIK TRANSAKSI (DATABASE SUPABASE)
               </h3>
                 ...
                 <div key={tx.id} className="space-y-3 pt-2">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                       <div className="flex flex-col gap-1.5">
                          <span className="text-[7px] font-black text-slate-500 uppercase text-center">1. Odo Sebelum</span>
                       ...
                       <div className="flex flex-col gap-1.5">
                          <span className="text-[7px] font-black text-slate-500 uppercase text-center">2. Odo Sesudah</span>
                          <div className="w-full h-32 bg-slate-50 border border-slate-300 rounded-[2px] flex items-center justify-center overflow-hidden">
                       ...
                       <div className="flex flex-col gap-1.5">
                          <span className="text-[7px] font-black text-slate-500 uppercase text-center">3. Nota / Struk</span>
                          <div className="w-full h-32 bg-slate-50 border border-slate-300 rounded-[2px] flex items-center justify-center overflow-hidden">
               ...
               {reportTransactions && reportTransactions.length > 5 && (
                 <p className="text-[8px] text-muted-foreground italic text-center pt-4">-- Menampilkan 5 bukti pertama pada preview, seluruh bukti ({reportTransactions.length}) akan tersemat pada file PDF --</p>
               )}
            </div>
            ...
            <div className="mt-auto pt-20 grid grid-cols-2 gap-10">
               <div className="text-center">
                  <p className="text-[9px] font-bold uppercase mb-16 italic">Mengetahui,<br/>Manajer Unit Layanan</p>
                  <p className="text-[10px] font-black border-t border-slate-800 pt-1 uppercase">( ............................ )</p>
               </div>
               <div className="text-center">
                  <p className="text-[9px] font-bold uppercase mb-16 italic">Dibuat Oleh,<br/>Admin Pengawas Wilayah</p>
